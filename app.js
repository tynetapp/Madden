...
const FEED_PAGE_CAP={chirper:3, huddle:2};
...
function feedMoreBtn(kind){
  ...
  if (used>=cap) return '<div style="text-align:center;opacity:.45;font-size:12px;padding:14px 0 24px">You\u2019re caught up for the week.</div>';
  const busy=S._feedBusy===kind;
  return '<div style="padding:10px 16px 28px"><button class="btn sm" style="width:100%;background:rgba(255,255,255,.1)" '+(busy? 'disabled' : 'onclick="feedMore(\''+kind+'\')"')+'>'+(busy? 'Loading\u2026' : (kind==="chirper"? "Show more posts" : "More threads"))+' <span style="opacity:.55">('+(cap-used)+' left this week)</span></button></div>';
}
...
async function feedMore(kind){
  try{
    ...
    if (kind==="chirper" && j && Array.isArray(j.chirps)){
      ...
      S.world.chirps=[...S.world.chirps, ...fresh].slice(0,64); chEnsureCids();
      ...
      S.world.huddle=[...S.world.huddle, ...fresh].slice(0,28);
...
async function chThreadPen(c, opt){
  ...
  const scale = L<=1? "BUZZ LEVEL 1 (a nobody): the base fold is 2-3 ordinary replies; nobody notices anything he does — react is 0-2 more ORDINARY replies to the post that do not mention him at all."
    : L<=4? "BUZZ LEVEL "+L+" (local): base is 5; react is 3-5 — a fan or two notices ('wait the QB liked this??'), the original poster may react once, no media."
    : L<=7? "BUZZ LEVEL "+L+" (known): base 5; react 5 — the poster reacts, fans pile in, one invented aggregator or beat account screenshots it or quotes him."
    : L<=9? "BUZZ LEVEL "+L+" (big): base 5; react 5 including an aggregator and a media account; his like or reply IS a story; also return a cascade with likes and replies jumping (hundreds to a few thousand) and 1-2 followup chirps by other accounts about the moment."
    : "BUZZ LEVEL "+L+" (enormous): base 5; react 5 and every one of them is about HIM; return a cascade — likes and replies explode (thousands+), and 2-3 followup chirps from media, teammates, or rival fans about the moment.";
  const sys="You write the reply section of a fake social platform (Chirper) in an NFL life sim. NEVER use real-world journalists, media personalities, or celebrities; only players and coaches from this save may be real, everyone else is invented. "+careerFactsLine()+" "+accountVoiceLaw()+" No em dashes. Numbers in digits. Mixed tones, real internet texture, no two replies alike, no handle used twice, never reuse these handles already in the thread: "+JSON.stringify(existing)+". Output STRICT JSON only, no fences: {\"base\":[],\"react\":[],\"cascade\":{\"likes\":0,\"replies\":0,\"followups\":[]}} where every reply and followup is {\"a\":\"name\",\"h\":\"@handle\",\"g\":\"m|f|x\",\"vf\":0,\"x\":\"text\"} (g: m male, f female, x fan or brand account; vf 1 ONLY for teammates, media outlets, and official accounts). Omit what was not asked for (empty arrays).";
  const user=(own? "HIS OWN POST by "+me+" ("+S.handle+"): \""+c.t+"\". " : "Original post by "+(c.n||"someone")+" ("+(c.h||"")+"): \""+c.t+"\". ")+
    ...
    (opt.base? (own? "WRITE base: the top replies UNDER HIS POST as they stand (replies to him, scaled to his buzz). " : "WRITE base: the 5 MOST POPULAR replies under the post as they stood BEFORE he touched it — they never mention him unless the post itself is about him. ") : "Do NOT write base (empty). ")+
    ...
    ((opt.react && L>=8)? "Also write cascade. " : "cascade stays zero and empty. ")+"Write it now.";
  const out=await callAI(sys, user, 1600);
  ...
  return { base:clean(j.base).slice(0, own? 8 : 5), react:clean(j.react).slice(0,5), cascade:(j.cascade&&typeof j.cascade==="object")? j.cascade : {likes:0,replies:0,followups:[]} };
}
...
async function aiThreadJoin(c, mine){
  try{
    ...
    return (j && (j.before||j.after||j.toMine))? {before:(j.before||[]).slice(0,4), after:(j.after||[]).slice(0,3), toMine:(j.toMine||[]).slice(0,5)} : null;
  }catch(e){ return null; }
}
